...
public extension Templates {
    ...
    struct Menu<Label: View>: View {
        ...
        /// macadamia local patch (vendored from aheze/Popovers 1.3.2): `@StateObject`
        /// instead of the upstream `@ObservedObject`. With `@ObservedObject` the inline
        /// `MenuModel()` is re-created every time the parent view's `body` re-evaluates,
        /// which resets `present` to false and dismisses an open menu. That manifested as
        /// the Receive/Send popover snapping shut on the first tap after launch, while the
        /// app was still re-rendering from async startup work (Nostr relay state, SwiftData
        /// @Query refresh, exchange-rate fetch). `@StateObject` makes SwiftUI own the model
        /// for the view's lifetime so the presentation state survives those re-renders.
